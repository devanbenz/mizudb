...
use datafusion::logical_expr::{DdlStatement, DmlStatement, EmptyRelation, LogicalPlan, LogicalPlanBuilder, TableSource};
...
impl MizuDB {
    async fn new(db_path: String) -> datafusion::error::Result<Self> {
        ...
        let url_path = format!("file://{}", path.as_str());
        let table_path = format!("file://{}/", path.as_str());
        let catalog = Arc::new(MizuCatalog::new());
        let object_store = Arc::new(MizuObjectStore::new(path.as_str()).await.map_err(|e| DataFusionError::External(Box::new(e)))?);
        let object_store_registry = Arc::new(MizuObjectStoreRegistry::with_default_store(
            Arc::new(object_store),
            Url::try_from(url_path.as_str()).unwrap(),
        ));
        ...
        let catalog_table_provider = Arc::new(ListingTable::try_new(
            ListingTableConfig::new(ListingTableUrl::parse(&table_path)?)
        ...
        let session_config = SessionConfig::new().with_default_catalog_and_schema(DEFAULT_CATALOG, DEFAULT_SCHEMA);
    ...
    async fn load_db(catalog: Arc<dyn CatalogProvider>, catalog_table_provider: Arc<dyn TableProvider>) -> datafusion::error::Result<Self> {
        Ok(Self {
            catalog,
            catalog_table_provider,
        ...
        match self.ctx().state().statement_to_plan(stmt).await? {
            LogicalPlan::Ddl(ddl) => match ddl {
                DdlStatement::CreateMemoryTable(stmt) => {
                    // TODO: Create the table on disk in the catalog
                    let schema =
                        stmt.name.schema().or_else(|| Some(DEFAULT_SCHEMA)).unwrap();
                    ...
                    let listing_table_provider = ListingTable::try_new(
                        ListingTableConfig::new(ListingTableUrl::parse(&self.table_path)?)
                            .with_listing_options(ListingOptions::new(Arc::new(
                                ParquetFormat::new(),
                            )))
                    ...
                    let table_source = Arc::new(DefaultTableSource::new(self.catalog_table_provider.clone()));
                    let catalog_input = LogicalPlanBuilder::values(vec![vec![lit(schema), lit(stmt.name.table())]])?.project(vec![
                        col("column1").alias("schema_name"),
                        col("column2").alias("table_name"),
                    ])?.build()?;
                    let logical_plan = LogicalPlanBuilder::insert_into(catalog_input, TableReference::full(DEFAULT_CATALOG, DEFAULT_SCHEMA, "mizudb_store"), table_source, InsertOp::Append)?.build()?;
                    ...
                    println!(
                        "[CREATE TABLE] resolving into catalog={:?}, session default catalog/schema={:?}",
                        stmt.name.catalog(),
                        self.ctx().state().config().options().catalog.default_catalog,
                    );
...
async fn main() -> datafusion::error::Result<()> {
    let path = std::env::args().nth(1).unwrap_or_else(|| DEFAULT_DB_PATH.to_string());
